...
public class MapsFragment extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //hospital =new MarkerOptions().position(new LatLng(29.8619,77.8932)).question("IIT Roorkee Hospital");
    }
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.fab_arc_menu_5:
                ...
                iconDrawable = R.drawable.ic_account_balance;
                break;
        }
    ...
    private void flyTO()
    {
        mMap.moveCamera(CameraUpdateFactory.newCameraPosition(MapsFragment.IITR));
    }
    public void displayAll() {
        mMap.clear();
        final float half = (float) 0.5;
        for (MarkerValue place : eventList) {
            Marker marker = mMap.addMarker(new MarkerOptions().position(place.getLatLng()).title(place.getTitle()).icon(BitmapDescriptorFactory.fromResource(R.drawable.ic_stars)).anchor(half, half));
            marker.setTag(place);
        }
        for (MarkerValue place : eateriesList) {
            Marker marker = mMap.addMarker(new MarkerOptions().position(place.getLatLng()).title(place.getTitle()).icon(BitmapDescriptorFactory.fromResource(R.drawable.ic_action_restaurant)).anchor(half, half));
            marker.setTag(place);
        }
        for (MarkerValue place : accomodationList) {
            Marker marker = mMap.addMarker(new MarkerOptions().position(place.getLatLng()).title(place.getTitle()).icon(BitmapDescriptorFactory.fromResource(R.drawable.ic_home)).anchor(half, half));
            marker.setTag(place);
        }
        for (MarkerValue place : atmlist) {
            Marker marker = mMap.addMarker(new MarkerOptions().position(place.getLatLng()).title(place.getTitle()).icon(BitmapDescriptorFactory.fromResource(R.drawable.ic_account_balance_wallet)).anchor(half, half));
            marker.setTag(place);

        }
        for (MarkerValue place :hospitallist) {
            Marker marker = mMap.addMarker(new MarkerOptions().position(place.getLatLng()).title(place.getTitle()).icon(BitmapDescriptorFactory.fromResource(R.drawable.ic_account_balance)).anchor(half, half));
            //    marker.setTag(place.getQuestion());
